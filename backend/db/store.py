...
class SupabaseStore:
    """Uses a single `documents` table (id, collection, created_at, data jsonb) — created by the
    P1.2 migration. Kept API-compatible with SqliteStore."""
    backend = "supabase"
    ...
    def __init__(self):
        ...
        self.client.table("documents").select("id").limit(1).execute()  # fail fast
    ...
    def list(self, collection: str, limit: int = 20) -> List[Dict[str, Any]]:
        res = (self.client.table("documents").select("id,data").eq("collection", collection)
    ...
    def count(self, collection: str) -> int:
        res = (self.client.table("documents").select("id", count="exact")
